...
if market_open:
    ...
    # CSS @keyframes countdown — pure CSS, no JS, no iframe sandbox issues
    countdown_html = f"""
    <style>
    .top-shell {{
        background: linear-gradient(90deg,#1a1c20,#1d1e21);
    ...
    .sync-wrap {{
        ...
        justify-content:center; width:100%; height:62px;
    }}
    .sync-label {{ font-size:11px; color:#99a1ab; font-weight:700; letter-spacing:0.05em; margin-bottom:2px; }}
    .sync-unit  {{ font-size:11px; color:#99a1ab; margin-top:1px; }}
    @property --t {{
        syntax: "<integer>";
        initial-value: {seconds_left};
        inherits: false;
    }}
    @keyframes cd {{
        from {{ --t: {seconds_left}; }}
    ...
    .cd-num {{
        font-size:26px; font-weight:900; color:#39a4ff; line-height:1;
        animation: cd {seconds_left}s steps({seconds_left}, end) 1 forwards;
        counter-reset: cd var(--t);
    }}
    .cd-num::after {{ content: counter(cd); }}
    </style>
    <div class="top-shell">
      <div class="sync-wrap">
    ...
    # Weekend-aware next market open calculation
    ...
    .sync-label {{ font-size:11px; color:#99a1ab; font-weight:700; letter-spacing:0.05em; margin-bottom:2px; }}
    .sync-unit  {{ font-size:11px; color:#99a1ab; margin-top:1px; }}
    .sync-countdown {{ font-size:26px; font-weight:900; color:#39a4ff; line-height:1; }}
    </style>
    <div class="top-shell">
      <div class="sync-wrap">
